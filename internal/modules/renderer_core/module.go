...
// MQTTClient is the broker surface the module needs. *mqttserver.Client
...
// ModuleConfig configures the shared renderer module scaffolding.
// Driver-specific configuration (pipelines, audio outputs, crossfade)
// lives in the renderer packages that construct the Driver.
...
// Module is the shared MQTT renderer module: it wires an Engine and a
// Driver to the command/reply/state/presence topics, debounces state
// publishes, and consumes async driver events. Renderer packages
// (renderer_gstreamer, renderer_mpv, ...) construct their Driver and
...
// NewModule creates a renderer module around an already-constructed
// driver.
func NewModule(log *zap.Logger, client MQTTClient, cfg ModuleConfig, driver Driver) (*Module, error) {
	...
	engine := NewEngine(cfg.NodeID, cfg.Name, driver)
	if cfg.Volume > 0 {
...
func (m *Module) Run(ctx context.Context) error {
	...
	defer func() {
		...
		// synchronous, bounded teardown so the audio server observes a
		// clean client disconnect before the process exits. Without that
		// it only sees a socket EOF and leaves stale node / buffer state
		// in the daemon — known to accumulate across nightly container
		// restarts and eventually break audio for all clients.
	...
	// Drive EOS / error / warning / audio-down handling from the driver's
	// async events when supported. This is what replaces the old
...
// heuristic), errors and warnings get logged, audio-down events surface in
// the renderer state.
func (m *Module) consumeDriverEvents(ctx context.Context, events <-chan DriverEvent) {
	for {
		select {
		case <-ctx.Done():
			return
		case ev, ok := <-events:
			...
			switch ev.Kind {
			case DriverEventEOS:
				m.log.Debug("driver EOS — advancing queue")
				m.engine.AdvanceAfterEnd()
				if m.config.PublishState {
					m.scheduleStatePublish()
				}
			case DriverEventError:
				m.log.Warn("driver playback error", zap.String("message", ev.Message))
				// Treat unrecoverable playback errors like EOS for now —
				// advance to the next track rather than wedging on the
				// broken one. If the audio backend is the underlying
				// issue, the driver reports that separately.
				m.engine.AdvanceAfterEnd()
				if m.config.PublishState {
					m.scheduleStatePublish()
				}
			case DriverEventWarning:
				// Driver-level warnings (sink fallbacks, stream quirks).
				// Logged loudly so misroutings and stream compatibility
				// issues are visible.
				m.log.Warn("driver warning", zap.String("message", ev.Message))
			case DriverEventAudioDown:
